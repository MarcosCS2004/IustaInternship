...
input_csv = "anwaelte_kombiniert.csv"
output_csv = "filtered_results_anwalt.csv"
...
with sync_playwright() as playwright:
    ...
    with open(output_csv, 'a', encoding='utf-8', newline='') as output_file:
        ...
        if not file_exists:
            writer.writerow(['Name', 'Address', 'LinkedIn', 'Website', 'Email', 'Lawyers', 'Lawfields'])  # Write header only once

        with open(input_csv, 'r', encoding='utf-8') as input_file:
            reader = csv.DictReader(input_file)
            for row in reader:
                url = row['anwalt_link'].strip()
                lawfields = row.get('specialities', 'No lawfields provided').strip()
                ...
                try:
                    ...
                    page.screenshot(path=f"error_page.png")

    browser.close()
    print(f"✔️ Done. Results appended to {output_csv}")
