...
def contains_relevant_info(text):
    ...
    address_keywords = ['address', 'dirección', 'direccion', 'street', 'avenue', 'calle',
                        'adresse', 'straße', 'strasse', 'platz', 'hausnummer']
    
    name_keywords = ['law firm', 'abogado', 'attorney', 'firma', 'law office',
                     'anwalt', 'rechtsanwalt', 'kanzlei', 'jurist']
...
def analyze_links(csv_path, link_column, output_csv):
    ...
    print(f"Analysis complete. {len(result_df)} links with relevant information saved to '{output_csv}'.")
