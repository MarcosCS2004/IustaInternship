...
def matches_criteria(text):
    ...
    # For traffic
    traffic_match = any(re.search(rf'\b{keyword}\b', text_lower) for keyword in TRAFFIC_KEYWORDS)
    
    # For Full Service (uses regex)
    full_service_match = any(re.search(keyword, text_lower) for keyword in FULL_SERVICE_KEYWORDS)
    
    # For licenses
    license_match = any(re.search(rf'\b{keyword}\b', text_lower) for keyword in LICENSE_KEYWORDS)
                ...
                for listing in listings:
                    ...
                    if speciality_element:
                        ...
                        if matches_criteria(speciality):
                            link_element = listing.query_selector("h3 a") or listing.query_selector("a[href]")
                            if link_element:
                                href = link_element.get_attribute("href")
                                title = link_element.get_attribute("title") or "No title"
                                f.write(f"Name: {title} | Speciality {speciality} | URL: {href}\n")
                                print(f"Record found: {title} - {speciality}")
                            else:
                                f.write(f"Specialty: {speciality} | Link not available\n")
